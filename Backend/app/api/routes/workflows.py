...
async def get_workflow(workflow_id: str) -> WorkflowOutput:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Database service unavailable"
        )
    ...
    return WorkflowOutput(
        ...
        audio_output=workflow.get("audio_output"),
...
async def delete_workflow(
    workflow_id: str,
    purge_media: bool = Query(
        default=True, description="Also delete stored media files and media records"
    ),
):
    """Soft delete a workflow by ID, with optional media purge."""
    start_time = time.time()
    log_request_start(
        logger, "DELETE", f"/api/v1/workflows/{workflow_id}", {"purge_media": purge_media}
    )
...
async def continue_workflow(workflow_id: str, background_tasks: BackgroundTasks):
    ...
    if not workflow:
        duration_ms = (time.time() - start_time) * 1000
        log_request_end(
            logger, "POST", f"/api/v1/workflows/{workflow_id}/continue", 404, duration_ms
        )
        raise HTTPException(status_code=404, detail="Workflow not found")
    if workflow.get("deleted_at"):
        duration_ms = (time.time() - start_time) * 1000
    ...
    if workflow.get("status") != "failed":
        duration_ms = (time.time() - start_time) * 1000
        log_request_end(
            logger, "POST", f"/api/v1/workflows/{workflow_id}/continue", 400, duration_ms
        )
        raise HTTPException(
            status_code=400,
...
async def purge_workflow_media(workflow_id: str):
    ...
    if not workflow:
        duration_ms = (time.time() - start_time) * 1000
        log_request_end(
            logger, "POST", f"/api/v1/workflows/{workflow_id}/purge-media", 404, duration_ms
        )
        raise HTTPException(status_code=404, detail="Workflow not found")
    ...
    try:
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_request_end(
            logger, "POST", f"/api/v1/workflows/{workflow_id}/purge-media", 200, duration_ms
        )
        return {
