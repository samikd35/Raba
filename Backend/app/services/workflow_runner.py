...
class WorkflowRunner:
    ...
    async def run_workflow(
        self, workflow_id: str, input_overrides: Optional[dict[str, Any]] = None
    ) -> dict[str, Any]:
        ...
        initial_state = cast(VideoGenerationState, self._build_initial_state(workflow))
        if input_overrides:
            # Merge any user-provided overrides (e.g., selected video model)
            initial_state = cast(
                VideoGenerationState,
                {**dict(initial_state), **input_overrides},
            )

        log_subheader(logger, "Initial State")
        log_key_value(logger, "workflow_id", workflow_id)
        log_key_value(logger, "topic", (initial_state.get("topic") or "")[:80])
        log_key_value(
            logger,
            "duration",
            f"{initial_state.get('duration_seconds', 0)}s",
        )
        log_key_value(logger, "hitl_mode", initial_state.get("hitl_mode"))
        ...
        try:
            ...
            config = cast(Any, {"recursion_limit": 100})
            ...
            if final_state.get("current_hitl_gate"):
                ...
                logger.info(
                    f"{Colors.BG_YELLOW}{Colors.BLACK} HITL {Colors.RESET} Awaiting user approval at gate: {gate}"
                )
                return final_state
    ...
    def _build_continue_state(self, workflow: dict[str, Any]) -> VideoGenerationState:
        ...
        base["audio_manifest"] = workflow.get("audio_output")
        base["audio_output"] = workflow.get("audio_output")
    ...
    async def run_workflow_continue(
        ...
        if workflow.get("status") != "failed":
            log_error_msg(
                logger, f"Cannot continue workflow {workflow_id}: status={workflow.get('status')}"
            )
            return {
                "error": f"Only failed workflows can be continued. Current status: {workflow.get('status')}"
            }

        log_workflow_event(
            logger,
            workflow_id,
            "Continue from failed",
            {
                "topic": workflow.get("topic", "")[:60],
        ...
        initial_state = cast(VideoGenerationState, self._build_continue_state(workflow))
        if input_overrides:
            initial_state = cast(
                VideoGenerationState,
                {**dict(initial_state), **input_overrides},
        ...
        try:
            ...
            if final_state.get("current_hitl_gate"):
                log_workflow_event(
                    logger,
                    workflow_id,
                    f"Paused at HITL gate: {final_state.get('current_hitl_gate')}",
                )
                return final_state
            ...
            if final_state.get("error"):
                ...
                await self._update_status(
                    workflow_id, WorkflowStatus.FAILED, error=final_state.get("error")
                )
                return final_state
...
async def run_workflow_background(workflow_id: str, video_model: Optional[str] = None) -> None:
    ...
    """
    logger.info(
        f"{Colors.BRIGHT_MAGENTA}[BACKGROUND]{Colors.RESET} Starting workflow: {workflow_id}"
    )
...
async def run_workflow_continue_background(workflow_id: str) -> None:
    ...
    """
    logger.info(
        f"{Colors.BRIGHT_MAGENTA}[BACKGROUND]{Colors.RESET} Continue workflow: {workflow_id}"
    )
