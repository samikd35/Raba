...
class VideoSegment(BaseModel):
    ...
    segment_number: int = Field(..., ge=0, description="Segment number (0-indexed)")
    segment_type: VideoSegmentType = Field(
        ..., description="Type of segment (initial or extension)"
    )
    duration_seconds: float = Field(..., gt=0, description="Duration of this segment in seconds")
    start_time: float = Field(default=0.0, ge=0, description="Start time in the final video")
    end_time: float = Field(default=0.0, ge=0, description="End time in the final video")
    prompt: str = Field(default="", description="Prompt used for this segment")
    generation_time_ms: int = Field(
        default=0, ge=0, description="Time taken to generate this segment"
    )
    used_reference_images: bool = Field(
        default=False, description="Whether reference images were used (initial segment only)"
    )
...
class VideoGenerationConfig(BaseModel):
    ...
    model: VideoModel = Field(
        default=VideoModel.VEO_3_1, description="Video generation model to use"
    )
    aspect_ratio: VideoAspectRatio = Field(
        default=VideoAspectRatio.PORTRAIT_9_16, description="Output video aspect ratio"
    )
    ...
    duration_seconds: int = Field(
        default=8, ge=4, le=8, description="Duration per segment (max 8s per Veo API)"
    )
    target_duration_seconds: int = Field(
        default=18, ge=8, le=60, description="Target total video duration"
    )
    enable_audio: bool = Field(default=False, description="Enable native audio generation")
    negative_prompt: Optional[str] = Field(
        default="text overlays, watermarks, logos, UI elements, low quality, blurry",
        description="Elements to avoid in the video",
    )
    max_retries: int = Field(default=3, ge=1, le=5, description="Max retry attempts on failure")
    poll_interval_seconds: int = Field(
        default=10, ge=5, le=30, description="Interval for polling video generation status"
    )
...
class ReferenceImageSelection(BaseModel):
    ...
    user_reference_url: Optional[str] = Field(
        default=None, description="User-provided reference image URL (highest priority)"
    )
    generated_image_urls: list[str] = Field(
        default_factory=list, description="URLs of generated images"
    )
    research_image_urls: list[str] = Field(
        default_factory=list, description="URLs of research images (lowest priority)"
    )
    selected_urls: list[str] = Field(
        default_factory=list, description="Final selected image URLs (max 3)"
    )
...
class GeneratedVideo(BaseModel):
    ...
    url: str = Field(..., description="Public URL of the generated video")
    storage_path: str = Field(default="", description="Path in Supabase Storage bucket")
    duration_seconds: float = Field(..., gt=0, description="Total duration of the video")
    resolution: VideoResolution = Field(
        default=VideoResolution.RES_720P, description="Video resolution"
    )
    aspect_ratio: VideoAspectRatio = Field(
        default=VideoAspectRatio.PORTRAIT_9_16, description="Video aspect ratio"
    )
    model_used: VideoModel = Field(
        default=VideoModel.VEO_3_1, description="Model used for generation"
    )
    segments: list[VideoSegment] = Field(
        default_factory=list, description="Segment breakdown for multi-segment videos"
    )
    total_segments: int = Field(default=1, ge=1, description="Total number of segments")
    audio_included: bool = Field(default=True, description="Whether native audio was generated")
    reference_images_used: list[str] = Field(
        default_factory=list, description="Reference image URLs used for generation"
    )
    generation_time_ms: int = Field(default=0, ge=0, description="Total time for video generation")
...
class VideoGeneratorOutput(BaseModel):
    ...
    video: GeneratedVideo = Field(..., description="Generated video with metadata")
    segments: list[VideoSegment] = Field(default_factory=list, description="All video segments")
    total_duration_seconds: float = Field(default=0.0, gt=0, description="Total video duration")
    total_generation_time_ms: int = Field(
        default=0, ge=0, description="Total time for all generation"
    )
    model_used: VideoModel = Field(default=VideoModel.VEO_3_1, description="Primary model used")
    ...
    reference_images: ReferenceImageSelection = Field(
        default_factory=ReferenceImageSelection, description="Reference images used"
    )
    prompt_used: str = Field(default="", description="Main prompt used for initial generation")
...
class VideoGenerationRequest(BaseModel):
    ...
    workflow_id: str = Field(..., description="Workflow ID for storage path")
    script_output: dict = Field(..., description="Script output with hook, scenes, CTA")
    all_images: list[str] = Field(default_factory=list, description="All available image URLs")
    user_reference_image_url: Optional[str] = Field(
        default=None, description="User-provided reference image URL"
    )
    research_images: list[str] = Field(default_factory=list, description="Research image URLs")
    generated_images: list[str] = Field(default_factory=list, description="Generated image URLs")
    tool_category: str = Field(
        default="surreal_realism", description="Tool category for visual style"
    )
    topic: str = Field(default="", description="Video topic for context")
    ...
    duration_seconds: int = Field(default=18, ge=8, le=60, description="Target video duration")
    enable_audio: bool = Field(default=True, description="Enable native audio generation")
    audio_manifest: Optional[AudioManifest] = Field(
        default=None,
        description="Audio manifest for audio-first generation",
    )
...
class HITLVideoFeedback(BaseModel):
    ...
    action: HITLVideoAction = Field(..., description="User action: approve or regenerate")
    feedback: Optional[str] = Field(default=None, description="General feedback for regeneration")
    pacing_feedback: Optional[str] = Field(default=None, description="Feedback on video pacing")
    transition_feedback: Optional[str] = Field(
        default=None, description="Feedback on scene transitions"
    )
    audio_feedback: Optional[str] = Field(default=None, description="Feedback on audio/dialogue")
    visual_feedback: Optional[str] = Field(default=None, description="Feedback on visual style")
    specific_timestamp: Optional[float] = Field(
        default=None, ge=0, description="Specific timestamp with issue (seconds)"
    )
