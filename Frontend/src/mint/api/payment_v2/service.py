...
class PaymentService:
    ...
    Assumes you pass in a Supabase client (e.g., supabase.Client) at init.
    """

    def __init__(self, use_service_role: bool = True):
        # In your app, initialize Supabase once and inject here.
        # Example elsewhere:
    ...
    def mark_payment_status(
        ...
        Update a payment row by tx_ref:
          ...
          - store tx_id (provider transaction id)
          - append/replace provider_payload (last seen)
        """
        # NOTE: if you’d like to keep a history, create a separate table for events.
        update_row = {
            "status": status,
            "tx_id": tx_id,
    ...
    def already_processed_transaction(self, tx_id: int) -> bool:
        """
        True if this provider transaction_id was already processed.
    ...
    def mark_transaction_processed(self, tx_id: int, tx_ref: str) -> dict:
        """
        Record a processed transaction_id to guard against webhook retries.
        Idempotent via PK/unique constraint.
        """
        # upsert ensures idempotency (no error if it already exists)
        res = (
            self.db.table("processed_transactions")
            .upsert({"transaction_id": tx_id, "tx_ref": tx_ref})
            .execute()
        )
    ...
    def get_credits_per_unit(self, currency: str) -> Decimal:
        ...
        rows = getattr(res, "data", None) or []
        if not rows:
            raise LookupError(f"No active credit rate for currency={currency}")
