...
app = FastAPI(
    ...
    openapi_tags=[
        ...
        {
            "name": "💰 Billing",
            "description": "Billing system for invoices, pricing, bulk purchases, and admin seat billing",
        },
        {
            "name": "👥 Cohorts",
            "description": "Cohort management for organizing members within organizations",
        },
        {
        ...
        },
        {
            ...
            "description": "Venture Builder coaching sessions, bookings, and profile management",
        },
        {
            "name": "MVP Requirements Generator",
            "description": "AI-powered MVP PRD generation with template routing and clarifying questions",
        },
    ],
)
...
# Feature Video Seen Tracking (per-user tracking for help video autoplay)
from .api.feature_video import feature_video_router

app.include_router(feature_video_router, prefix="/api/v1", tags=["Feature Videos"])
logger.info("✅ Feature Video Seen router registered successfully")
logger.info("📹 Feature Video endpoints:")
logger.info("   - GET /api/v1/feature-videos/seen (get all seen feature IDs)")
logger.info("   - POST /api/v1/feature-videos/seen (mark feature video as seen)")
...
# Billing and cohort management
from .api.billing.endpoints import router as billing_router
from .api.billing.cohort_endpoints import router as cohort_router

app.include_router(billing_router)
app.include_router(cohort_router)
logger.info("✅ Billing and cohort routers registered successfully")

# Demo request (Enterprise pricing page demo requests)
from .api.demo_request import demo_request_router

app.include_router(demo_request_router, prefix="/api", tags=["demo-request"])
logger.info("✅ Demo request router registered successfully")
logger.info("📧 Demo request endpoint: POST /api/demo-request")
...
try:
    from .api.cache.endpoints import cache_router
    app.include_router(cache_router, tags=["cache"])
    logger.info("✅ Cache health/stats router registered successfully")
    logger.info("📊 Cache endpoints:")
    logger.info("   - GET /api/cache/health (health check)")
    logger.info("   - GET /api/cache/stats (statistics)")
    logger.info("   - POST /api/cache/reconnect (reconnect to Redis)")
except Exception as e:
    logger.error(f"❌ Failed to register cache router: {e}")
...
# MVP Requirements Generator (AMRG) - PRD Generation
try:
    from src.mvp.mvp_req import get_amrg_router
    amrg_router = get_amrg_router()
    app.include_router(amrg_router)
    logger.info("✅ MVP Requirements Generator (AMRG) router registered successfully")
    logger.info("📋 AMRG endpoints:")
    logger.info("   - POST /mvp/projects/{project_id}/amrg/runs (start PRD generation)")
    logger.info("   - POST /mvp/amrg/runs/{run_id}/answers (submit answers)")
    logger.info("   - GET /mvp/amrg/runs/{run_id} (get run status)")
    logger.info("   - GET /mvp/projects/{project_id}/amrg/results (get PRD)")
    logger.info("   - GET /mvp/projects/{project_id}/amrg/history (version history)")
except Exception as e:
    logger.error(f"❌ Failed to register AMRG router: {e}")
    logger.info("MVP Requirements Generator will be available when dependencies are installed")

# Module 3 Bootstrap Route - Direct entry to Module 3 without completing Modules 1-2
try:
    from src.mvp.bootstrap.api.endpoints import router as bootstrap_router
    app.include_router(bootstrap_router, prefix="/api/v2")
    logger.info("✅ Module 3 Bootstrap router registered successfully")
    logger.info("🚀 Bootstrap endpoints:")
    logger.info("   - POST /api/v2/mvp/bootstrap/projects (create bootstrap project)")
    logger.info("   - GET /api/v2/mvp/bootstrap/projects/{project_id}/questions (get clarifying questions)")
    logger.info("   - POST /api/v2/mvp/bootstrap/projects/{project_id}/answers (submit answers)")
    logger.info("   - GET /api/v2/mvp/bootstrap/projects/{project_id}/enhanced-context (get context)")
    logger.info("   - PUT /api/v2/mvp/bootstrap/projects/{project_id}/enhanced-context/confirm (confirm context)")
    logger.info("   - GET /api/v2/mvp/bootstrap/projects/{project_id}/status (get status)")
except Exception as e:
    logger.error(f"❌ Failed to register Bootstrap router: {e}")
    logger.info("Module 3 Bootstrap will be available when dependencies are installed")

# Venture Builder System
logger.info("🔍 Attempting Venture Builder system integration...")
try:
    from .api.venture_builder import (
        invitations_router,
        expertise_router,
        profiles_router,
        sessions_router,
    ...
    from .api.venture_builder.exceptions import VBBaseException
    from .api.venture_builder.utils import vb_exception_handler
    ...
    # Register custom exception handler for VB exceptions
    app.add_exception_handler(VBBaseException, vb_exception_handler)

    logger.info("✅ Venture Builder routers registered successfully (11 routers)")
    logger.info("✅ Venture Builder exception handler registered")
except ImportError as e:
    logger.error(f"❌ Venture Builder import failed: {e}")
    logger.info("Venture Builder will be available when dependencies are installed")
except Exception as e:
    logger.warning(f"⚠️ Venture Builder integration failed: {e}")
    logger.info("Venture Builder will be available when integration is complete")
...
# MAV (Module 4) - Project Chat with RAG and Web Search
logger.info("🔍 Attempting MAV Project Chat integration...")
try:
    from src.mav.chat import chat_router as mav_chat_router
    app.include_router(mav_chat_router, prefix="/api/v1", tags=["Project Chat"])
    logger.info("✅ MAV Project Chat router registered successfully")
    logger.info("💬 Project Chat endpoints:")
    logger.info("   - POST /api/v1/mav/projects/{project_id}/threads (create thread)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/threads (list threads)")
    logger.info("   - GET /api/v1/mav/threads/{thread_id} (get thread)")
    logger.info("   - POST /api/v1/mav/threads/{thread_id}/messages (post message)")
    logger.info("   - GET /api/v1/mav/threads/{thread_id}/messages (get messages)")
    logger.info("   - GET /api/v1/mav/threads/{thread_id}/memory (get thread memory)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/chunk-stats (get chunk stats)")
except ImportError as e:
    logger.error(f"❌ MAV Project Chat import failed: {e}")
    logger.info("MAV Project Chat will be available when dependencies are installed")
except Exception as e:
    logger.warning(f"⚠️ MAV Project Chat integration failed: {e}")
    import traceback
    logger.error(traceback.format_exc())
    logger.info("MAV Project Chat will be available when integration is complete")

# MAV (Module 4) - Pitch Deck Generator
logger.info("🔍 Attempting MAV Pitch Deck Generator integration...")
try:
    from src.mav.Pitch import pitch_router as mav_pitch_router
    app.include_router(mav_pitch_router, prefix="/api/v1", tags=["Pitch Deck Generator"])
    logger.info("✅ MAV Pitch Deck Generator router registered successfully")
    logger.info("🎯 Pitch Deck endpoints:")
    logger.info("   - POST /api/v1/mav/projects/{project_id}/pitch-deck/generate (generate deck)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/pitch-deck (get deck)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/pitch-deck/versions (list versions)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/pitch-deck/status (get status)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/pitch-deck/preview (preview plan)")
except ImportError as e:
    logger.error(f"❌ MAV Pitch Deck Generator import failed: {e}")
    logger.info("MAV Pitch Deck Generator will be available when dependencies are installed")
except Exception as e:
    logger.warning(f"⚠️ MAV Pitch Deck Generator integration failed: {e}")
    import traceback
    logger.error(traceback.format_exc())
    logger.info("MAV Pitch Deck Generator will be available when integration is complete")

# MAV (Module 4) - GTM Strategy Generator
logger.info("🔍 Attempting MAV GTM Strategy Generator integration...")
try:
    from src.mav.Pitch.GTM.api.endpoints import router as mav_gtm_router
    app.include_router(mav_gtm_router, prefix="/api/v1", tags=["GTM Strategy Generator"])
    logger.info("✅ MAV GTM Strategy Generator router registered successfully")
    logger.info("🎯 GTM Strategy endpoints:")
    logger.info("   - POST /api/v1/mav/projects/{project_id}/gtm/generate (generate GTM)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/gtm (get GTM strategy)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/gtm/versions (list versions)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/gtm/status (get status)")
    logger.info("   - GET /api/v1/mav/projects/{project_id}/gtm/summary (get summary)")
except ImportError as e:
    logger.error(f"❌ MAV GTM Strategy Generator import failed: {e}")
    logger.info("MAV GTM Strategy Generator will be available when dependencies are installed")
except Exception as e:
    logger.warning(f"⚠️ MAV GTM Strategy Generator integration failed: {e}")
    import traceback
    logger.error(traceback.format_exc())
    logger.info("MAV GTM Strategy Generator will be available when integration is complete")
...
async def lookup_error_handler(request: Request, exc: LookupError):
    """
    Convert LookupError to a 422 Unprocessable Entity error.
    This handles cases where resources (like credit rates) are not found.
    """
    return JSONResponse(
        status_code=422,
