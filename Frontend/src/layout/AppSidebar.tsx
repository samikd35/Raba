...
const navItems: NavItem[] = [
  ...
  {
    icon: <ProblemDiscovery className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
    name: "Problem Discovery",
    subItems: [
      { name: "Problem Validator", path: "/workspace/problem-validator", pro: false }
      // ,{ name: "Problem Predictor", path: "/workspace/idea-refiner", pro: false }
      // ,{ name: "Problem Explorer", path: "/workspace/problem-explorer", pro: false }
    ],
  },


  {
    icon: <ValueProposition className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
    name: "Value Proposition Design",
    subItems: [
      { name: "Customer Understanding", path: "/workspace/projects", pro: false }
      , { name: "Market Findings Analysis", path: "/workspace/projects-questionnaire-completed", pro: false }
    ],
  },
  {
    ...
    subItems: [
      { name: "Business Model Innovation", path: "/workspace/projects-mvp", pro: false }
      , { name: "Product Requirement", path: "/workspace/product-requirement/projects", pro: false }
    ],
  },
   {
    ...
    subItems: [
      { name: "Ask Yuba AI Expert", path: "/workspace/projects-project-chat", pro: false }
  ...
  // {
  //   icon: <Team className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
  //   name: "Cofounder Matching",
  //   subItems: [
  //     { name: "My Profile", path: "/workspace/cofounder-matching/dashboard", pro: false },
  //     { name: "Potential Matches", path: "/workspace/cofounder-matching/matches", pro: false },
  //     { name: "Community", path: "/workspace/cofounder-matching/browse", pro: false }
  //   ],
  // },
  {
    icon: <Team className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
  ...
  },
  {
    ...
    path: "/workspace/credit-requests",
  },
  {
    icon: <Consultation className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
    name: "Book a consultation",
    path: "/workspace/book-a-consultation",
  },
  {
    icon: <Calendar className="w-5 h-5 text-brand-500 transition-colors duration-200 group-hover:text-brand-600 dark:text-brand-200 dark:group-hover:text-brand-300" />,
    name: "Venture Builder",
    subItems: [
      { name: "Sessions", path: "/workspace/vb-portal/sessions", pro: false },
      { name: "Earnings", path: "/workspace/vb-portal/earnings", pro: false },
      { name: "Settings", path: "/workspace/vb-portal/settings", pro: false },
    ],
  },
  // {
  //   icon: <CalenderIcon />,
...
const AppSidebar: React.FC = () => {
  ...
  const separatorIndices = [0, 4, 6, 7]; // Define separator positions for maintainability
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log("[AppSidebar] Sidebar status loaded:", {
        ...
        if (process.env.NODE_ENV === "development") {
          console.error("[AppSidebar] Failed to fetch sidebar status:", error);
        }
      }
    };
  ...
  const filteredNavItems = useMemo(() => {
    // Check if user is a venture builder
    const isVentureBuilder = user?.roles?.includes('venture_builder') || false;

    // Debug logging
    if (process.env.NODE_ENV === 'development') {
      console.log('🔍 Sidebar - User roles check:', {
    ...
    // Filter out Team Members for non-team users and Venture Builder for non-VB users
    let baseNavItems = navItems;

    if (user?.tenant_type !== "team") {
      baseNavItems = baseNavItems.filter((item) => item.name !== "Team Members");
    }

    if (!isVentureBuilder) {
      baseNavItems = baseNavItems.filter((item) => item.name !== "Venture Builder");
    }
      ...
      if (item.name === "Value Proposition Design" && item.subItems) {
        return {
          ...
          subItems: item.subItems.map((subItem) => {
            ...
            if (subItem.path === "/workspace/projects") {
              return {
                ...subItem,
            ...
            if (subItem.path === "/workspace/projects-questionnaire-completed") {
              return {
                ...subItem,
      ...
      if (item.name === "MVP & BM Development" && item.subItems) {
        return {
          ...
          subItems: item.subItems.map((subItem) => {
            ...
            if (subItem.path === "/workspace/projects-mvp") {
              return {
                ...subItem,
            ...
            if (subItem.path === "/workspace/product-requirement/projects") {
              return {
                ...subItem,
      ...
      if (item.name === "Market Validation" && item.subItems) {
        return {
          ...
          subItems: item.subItems.map((subItem) => {
            ...
            if (subItem.path === "/workspace/projects-project-chat") {
              return {
                ...subItem,
  ...
  }, [user?.tenant_type, user?.roles, unlockFlags]);
