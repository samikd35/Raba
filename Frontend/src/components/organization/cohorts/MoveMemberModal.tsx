...
interface MoveMemberModalProps {
    ...
    onMove: (targetCohortId: string) => Promise<void>;
}
...
export const MoveMemberModal = ({
    ...
    useEffect(() => {
        if (isOpen && tenantId && token) {
            fetchCohorts();
            setSelectedCohortId(null); // Reset selection on open
            setSearchQuery('');
        }
    ...
    const handleMove = async () => {
        if (!selectedCohortId || !member?.member_tenant_id) return;

        setIsSubmitting(true);
        try {
            // Use the new move API endpoint
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/api/cohorts/${tenantId}/${currentCohortId}/members/${member.member_tenant_id}/move`,
                {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${token}`,
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to move member');
            }

            toast.success(`${member.name} moved to ${selectedCohort?.name || 'new cohort'} successfully`);
            await onMove(selectedCohortId);
            onOpenChange(false);
        } catch (error: any) {
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px] max-h-[70vh] flex flex-col p-0 gap-0 overflow-hidden">
                <DialogHeader className="bg-brand-25 border-b border-brand-100 p-4 shrink-0">
                    <DialogTitle>
                        <span className="text-brand-500 text-lg font-semibold">Move Member to Another Cohort</span>
                    </DialogTitle>
                    <DialogDescription className="text-gray-600 text-sm">
                        Select a cohort to move <span className="font-medium text-gray-900">{member?.name || 'this member'}</span> to.
                    </DialogDescription>
                </DialogHeader>
