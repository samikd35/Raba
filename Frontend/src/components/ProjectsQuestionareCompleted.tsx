...
// Types based on the new API response format
interface Persona {
  id: string;
  name: string;
  description: string;
...
interface Project {
  ...
  questionnaires_generated_at: string | null;
}
...
// Cache configuration
const CACHE_KEY = 'completed_questionnaires_cache';
const CACHE_TIMESTAMP_KEY = 'completed_questionnaires_cache_timestamp';
...
async function fetchCompletedQuestionnaires(signal?: AbortSignal, skipCache = false): Promise<ProjectsResponse> {
  ...
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/vmp/projects/completed/questionnaires`, {
    method: 'GET',
    headers,
    signal,
  });
...
const ProjectCard = React.memo(({ project, index, onSelect }: { project: Project; index: number; onSelect: (project: Project) => void }) => {
  ...
  // Calculate statistics based on optimized backend counts
  const totalQuestionnaires = project.questionnaires_count ?? 0;
  const totalHypotheses = project.hypotheses_count ?? 0;
  const totalAssumptions = project.assumptions_count ?? 0;
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">Questionnaires</span>
              </div>
              <p className="text-2xl font-bold text-brand-500 dark:text-gray-100">{totalQuestionnaires}</p>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">Hypotheses</span>
              </div>
              <p className="text-2xl font-bold text-brand-500 dark:text-gray-100">{totalHypotheses}</p>
            </div>

            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700">
              <div className="flex items-center gap-2 mb-1">
                <BarChart3 className="h-4 w-4 text-gray-600 dark:text-gray-400" />
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">Assumptions</span>
              </div>
              <p className="text-2xl font-bold text-brand-500 dark:text-gray-100">{totalAssumptions}</p>
            </div>
          </div>
...
// Main Component
export default function ProjectsQuestionareCompleted({ path }: { path: string }) {
  ...
  const fetchData = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log('Fetching completed questionnaires from API...');
      }

      const response = await fetchCompletedQuestionnaires(abortControllerRef.current.signal, forceRefresh);
    ...
    filtered.sort((a, b) => {
      ...
      switch (filters.sortBy) {
        ...
        case 'updated_at':
          ...
          break;
        case 'questionnaires':
          comparison = (a.questionnaires_count ?? 0) - (b.questionnaires_count ?? 0);
          break;
      }
  ...
  const handleProjectSelect = useCallback((project: Project) => {
    ...
    router.push(`/${path}/market-research-analysis/${project.id}`);
  }, [router, path]);
  ...
  if (status === 'empty') {
    return (
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <ProjectsEmpty
          onGoToCustomerUnderstanding={() => router.push(`/${path}/projects`)}
          onStartValidationProject={() => router.push(`/${path}/problem-validator`)}
        />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
      ...
      <div className="mb-4">
        ...
        {/* Results count - guaranteed success state due to early returns */}
        <div className="mt-4 text-sm text-gray-600 dark:text-gray-400">
          Showing {filteredAndSortedProjects.length} of {projects.length} projects
        </div>
      </div>

      {/* Projects grid - guaranteed success state due to early returns */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
