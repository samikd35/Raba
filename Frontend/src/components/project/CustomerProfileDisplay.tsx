...
import { CustomerProfile, CustomerProfileItem } from '@/types/organization';
import { 
  User,
  Briefcase,
  AlertTriangle,
...
export interface CustomerProfileDisplayProps {
...
export function CustomerProfileDisplay({ 
  customerProfile, 
  title = "Customer Profile",
  readOnly = false 
}: CustomerProfileDisplayProps) {
  const { jobs_to_be_done, pains, gains } = customerProfile;

  const hasData = (jobs_to_be_done?.length > 0) || (pains?.length > 0) || (gains?.length > 0);
