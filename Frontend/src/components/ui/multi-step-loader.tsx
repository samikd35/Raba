  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        This Deep Research may take 10-15 min to complete
        </p>
      </div>
...
export const MultiStepLoader = ({
  ...
  loading,
  onCancel,
  totalDuration = 10 * 60 * 1000, // 10 minutes default
  progress,
  progressDetails,
}: {
  loadingStates: LoadingState[];
  ...
  // Use external progress when provided, otherwise use internal simulation
  useEffect(() => {
    if (progress !== undefined) {
      // Use external progress from ProcessingIndicator logic
      setProgressPercent(progress);
      
      // Calculate current step based on progress
      const stepProgress = progress / 100;
      const currentStep = Math.min(
        Math.floor(stepProgress * loadingStates.length),
  ...
  // Start progress simulation when loading becomes true (only if no external progress)
  useEffect(() => {
    if (!loading || isComplete || progress !== undefined) return;
  ...
  }, [loading, isComplete, totalDuration, progressMilestones, loadingStates.length, progress]);
    ...
    <AnimatePresence mode="wait">
      {loading && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm h-screen"
        >
          <motion.div
