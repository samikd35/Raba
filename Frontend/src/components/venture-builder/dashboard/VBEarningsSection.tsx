...
import { DollarSign, TrendingUp, Calendar, Loader2, AlertCircle, Filter } from 'lucide-react';
import { getMyEarnings } from '@/lib/api/venture-builder';
import { authService } from '@/services/authService';
import { toast } from 'react-hot-toast';
import type { EarningsResponse } from '@/types/ventureBuilder';

export default function VBEarningsSection() {
  ...
  const loadEarnings = async () => {
    try {
      ...
      if (!currentUser?.roles?.includes('venture_builder') && !currentUser?.roles?.includes('admin')) {
        throw new Error('You must be a Venture Builder to view earnings.');
      }
