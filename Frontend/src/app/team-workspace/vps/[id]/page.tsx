...
export default function VPSPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // const featureConfig = getFeatureVideoConfig(FEATURE_IDS.VPS);
            ...
            <TabsList className="grid grid-cols-2 w-full bg-gray-100 dark:bg-gray-700">
              {availablePersonas.map((personaId, index) => (
                <TabsTrigger 
                  key={personaId} 
                  value={personaId}
                  className="flex items-center gap-2 text-gray-700 dark:text-gray-300 data-[state=active]:bg-white dark:data-[state=active]:bg-gray-600 data-[state=active]:text-brand-600 dark:data-[state=active]:text-brand-400"
                >
  ...
  const fetchVPSData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/mvp/projects/${projectId}/vps/v1`, {
        method: 'GET',
        headers: {
  ...
  const generateVPS = useCallback(async (projectId: string, token: string, creativityLevel: number = 0.7) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v2/mvp/projects/${projectId}/vps/v1/generate`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
  ...
  const handleSaveChanges = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(errorData.message || 'Invalid request format. Please check your input.');
        }
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      ...
      // Fetch the updated VPS data from backend
      await fetchVPSData();
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      ...
      const result = await generateVPS(projectId, token, 0.7);

      if (process.env.NODE_ENV === 'development') {
        console.log('✅ VPS regenerated successfully:', result);
      }

      // Fetch the updated VPS data
      await fetchVPSData();
  ...
  }, [isAuthenticated, token, projectId, router, generateVPS, fetchVPSData]);
  ...
  const handleContinueToBMC = useCallback(async () => {
    ...
    router.push(`/team-workspace/bmc/${projectId}`);
  }, [router, projectId]);
          ...
          <div className="flex items-center justify-center min-h-[500px]">
            ...
            >
              <div className="relative mb-6">
                <div className="w-16 h-16 mx-auto bg-brand-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                ...
                <motion.div
                  ...
                  className="absolute inset-0 w-16 h-16 mx-auto bg-brand-300 dark:bg-gray-600 rounded-full"
                />
              </div>
              ...
              <div className="flex items-center justify-center space-x-1">
                <motion.div
                  ...
                  className="w-1.5 h-1.5 bg-brand-500 dark:bg-gray-600 rounded-full"
                />
                <motion.div
                  animate={{ opacity: [0.4, 1, 0.4] }}
      ...
      <div>
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-8">
          <div className="flex items-center justify-center min-h-[500px]">
            ...
            >
              <div className="relative mb-6">
                <div className="w-20 h-20 mx-auto bg-brand-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                ...
                <motion.div
                  ...
                  transition={{ duration: 2, repeat: Infinity }}
                  className="absolute inset-0 w-20 h-20 mx-auto bg-brand-200 dark:bg-gray-700 rounded-full opacity-20"
                />
              </div>
              
              <h3 className="text-xl font-semibold text-brand-500 dark:text-white mb-3">
  ...
  if (vpsDataList.length === 0 || !currentVpsData) {
    ...
    return (
      <div>
        <PageBreadcrumb pageTitle="Design Your Value Proposition Statement" />
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-8">
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            <div className="w-12 h-12 border-4 border-brand-200 border-t-brand-500 rounded-full animate-spin"></div>
            <div className="text-center">
              <p className="text-lg font-medium text-brand-500 dark:text-white">Loading Value Proposition Statement</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">Preparing your statement data...</p>
            ...
            <div className="text-center max-w-md">
              ...
              {isProjectNotFoundError ? (
                <div className="space-y-4">
                  <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 text-sm text-red-800 dark:text-red-200 text-left">
                ...
                <div className="space-y-4">
                  <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4 text-sm text-orange-800 dark:text-orange-200 text-left">
                    <strong>What this means:</strong>
                    <ul className="mt-2 space-y-1">
                  ...
                  <div className="flex gap-3 justify-center">
                    <Button onClick={() => fetchVPSData(true)} className="bg-brand-600 hover:bg-brand-700">
  ...
  return (
    <div>
      {/* <FeatureVideoOverlay
        featureId={FEATURE_IDS.VPS}
        youtubeId={featureConfig.youtubeId}
        resourcesHref={featureConfig.resourcesHref}
        title={featureConfig.title}
      /> */}
      <PageBreadcrumb pageTitle="Value Proposition Statement" titleSuffix={<CreditCostBadge cost={15} />} />
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <p className="text-sm text-brand-500 dark:text-gray-300 flex items-center gap-2 px-4 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-600 py-2 border rounded-lg justify-center">
            ...
            <div className="flex items-center gap-3">
              ...
              {isEditing ? (
                ...
                <Button 
                  ...
                  className="border-brand-300 dark:border-gray-600 text-brand-600 dark:text-brand-300 hover:bg-brand-50 dark:hover:bg-gray-800"
                >
              ...
              <CardContent className="px-4">
                ...
                {isEditing && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    className="mb-2 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isEditing ? (
                        <Input
                          ...
                          className="bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-full px-4 py-2 text-md font-medium focus:ring-2 focus:ring-brand-500 focus:border-transparent"
                      ...
                      {isEditing ? (
                        <div className="col-span-7">
                          <Input
                            ...
                            className="bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-full px-4 py-2 font-medium  focus:ring-2 focus:ring-brand-500 focus:border-transparent"
