...
export default function CustomerProfileV2Page({
  ...
  const loadSelections = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes("404") || err.message.includes("not found")) {
        ...
        setStats(null);
        // Generate customer profile instead of routing
  ...
  const handleBackToProject = () => {
    router.push(`/team-workspace/market-research-analysis/${projectId}`);
  };
...
const generateCustomerProfile = useCallback(
  ...
  const handleContinueToVPC = useCallback(async () => {
    ...
    try {
      router.push(`/team-workspace/value-map-selections/${projectId}`);
    } catch (err) {
  ...
  if (error) {
    return (
      <div>
        <PageBreadcrumb pageTitle="Customer Profile Selections" />
