...
const InsightsLoading = React.memo(() => (
  <div className="flex items-center justify-center min-h-[calc(100vh-10rem)] bg-background dark:bg-gray-900">
    <div className="text-center max-w-lg mx-auto p-8">
      {/* Spinner */}
      <div
        className="mx-auto mb-2 h-8 w-8 rounded-full border-2 border-brand-500/30 dark:border-brand-700/50 border-t-brand-600 dark:border-t-brand-400 animate-spin"
        role="status"
  ...
  <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
    <div className="text-center max-w-md mx-auto p-8">
      ...
      <div className="flex gap-3 justify-center">
        <Button onClick={onRetry} className="bg-brand-600 hover:bg-brand-700 dark:bg-brand-500 dark:hover:bg-brand-600">
          <RefreshCw className="h-4 w-4 mr-2" />
          Try Again
        </Button>
        <Button onClick={onBack} variant="outline" className="border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800">
...
const ChatMessageDisplay = React.memo(({ insight }: { insight: BackendInsight & { content: ChatMessageContent } }) => {
  ...
  const iconClass = isUser ? 'bg-brand-500' : 'bg-gray-500 dark:bg-gray-600';

  return (
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`flex gap-3 max-w-4xl ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${iconClass}`}>
...
const MarketInsightsSection = React.memo(({ insight }: { insight: BackendInsight & { content: StructuredInsightContent } }) => {
  ...
  return (
    <section className="mb-8">
      ...
      <h2 className="text-xl font-bold text-brand-600 dark:text-brand-300">
       Emerging Key Insights
      </h2>

       <p className="text-sm text-brand-600 dark:text-brand-400 leading-relaxed italic">
...
const ListItem = React.memo(({ 
  ...
  <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
    <div className="flex gap-3">
          <div className={`shrink-0 w-6 h-6 bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 rounded-full text-xs font-bold flex items-center justify-center mt-1 border border-brand-200 dark:border-brand-700`}>
        {index + 1}
      </div>
...
const AnalysisSection = React.memo(({ 
  ...
  <div className="p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
    <h4 className="font-bold text-brand-600 dark:text-brand-300 flex items-center gap-2 text-xl">
      {/* <section.icon className={`h-4 w-4 ${section.color}`} /> */}
      {section.title}
    </h4>
    {section.description && (
      <p className="text-sm text-brand-600 dark:text-brand-400 mb-4 leading-relaxed italic">
        {section.description}
      </p>
    )}
    <div className="space-y-3">
...
const ActionableInsights = React.memo(function ActionableInsights({ reportId: propReportId }: { reportId?: string }) {
  ...
  const handleBackToReport = useCallback(() => {
    router.push(`/team-workspace/problem-validator/${reportId}/results`);
  }, [router, reportId]);
  ...
  return (
    <div key="insights-content" >
      ...
      <div className="max-w-6xl mx-auto">
        <Card className="p-6 shadow-sm bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
