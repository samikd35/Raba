...
import { useBMCv2 as useBMC, BMCBlockName, BMCEditItem } from "@/hooks/useBMCv2";
...
export default function BMCPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const featureConfig = getFeatureVideoConfig(FEATURE_IDS.BMC_V2);
  ...
  return (
    <div>
      {/* <PageBreadcrumb pageTitle={projectName ? `${projectName} - Business Model Canvas v2` : "Business Model Canvas v2"} /> */}
      <FeatureVideoOverlay
        featureId={FEATURE_IDS.BMC_V2}
        youtubeId={featureConfig.youtubeId}
        resourcesHref={featureConfig.resourcesHref}
        title={featureConfig.title}
      />
      <PageBreadcrumb pageTitle={"Post Solution Critique Business Model Canvas"} titleSuffix={<CreditCostBadge cost={20} />} />
