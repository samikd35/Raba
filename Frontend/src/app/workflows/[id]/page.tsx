...
export default function WorkflowDetailPage() {
    ...
    const audioUrl = useMemo(
        () => workflow?.audio_url || (workflow?.audio_output as any)?.merged_audio_url || null,
        [workflow?.audio_url, workflow?.audio_output]
    )
    ...
    const steps = useMemo(() => {
        ...
        const isAwaitingGate = Boolean(workflow?.current_hitl_gate)
        const canAdvance = !isPending && !isTerminal && !isAwaitingGate

        const hasTool = Boolean(workflow?.tool_selection)
        const hasResearch = Boolean(workflow?.research_output)
        const hasScript = Boolean(workflow?.script_output)
        const hasCharacterSheet = Boolean(workflow?.character_reference_sheet)
        const hasImages = imageUrls.length > 0
        const hasVideo = Boolean(workflow?.video_url)

        const rawLeadCharacter = (workflow?.script_output as any)?.lead_character
        const leadCharacter =
            typeof rawLeadCharacter === 'string' ? rawLeadCharacter.trim() : ''
        const characterRequired = Boolean(leadCharacter)
        const characterStepCompleted = hasCharacterSheet || (hasScript && !characterRequired)

        // Calculate research sources count from various possible structures
        const researchSourcesMeta = (() => {
            if (!workflow?.research_output) return 0
            const ro = workflow.research_output

            if (ro.strategy_used === 'creative') {
                return 'Creative'
            }

            // Check total_sources field first
            if (ro.total_sources) return `${ro.total_sources} sources`
            ...
            if (ro.research_findings && Array.isArray(ro.research_findings)) {
                ...
                if (citationUrls.size > 0) return `${citationUrls.size} sources`
            }

            // Check direct sources array
            if (ro.sources && Array.isArray(ro.sources)) {
                return `${ro.sources.length} sources`
            }
        ...
        return [
            {
                ...
                status: hasTool
                    ? 'completed'
                    : workflow?.current_hitl_gate === 'tool_selection'
                        ? 'waiting'
                        : canAdvance
                            ? 'running'
                            : 'pending',
            ...
            },
            {
                ...
                status: hasResearch
                    ? 'completed'
                    : hasTool && workflow?.current_hitl_gate === 'research'
                        ? 'waiting'
                        : hasTool && canAdvance
                            ? 'running'
                            : 'pending',
                meta: hasResearch ? `${researchSourcesMeta}` : undefined
            },
            {
                id: 'script',
                label: 'Script',
                status: hasScript
                    ? 'completed'
                    : hasResearch && workflow?.current_hitl_gate === 'script'
                        ? 'waiting'
                        : hasResearch && canAdvance
                            ? 'running'
                            : 'pending',
                meta: hasScript && workflow?.script_output?.viral_score !== undefined
            ...
            },
            {
                id: 'character',
                label: 'Character',
                status: characterStepCompleted
                    ? 'completed'
                    : hasScript && workflow?.current_hitl_gate === 'character_reference'
                        ? 'waiting'
                        : hasScript && canAdvance
                            ? 'running'
                            : 'pending',
                meta: hasCharacterSheet
                    ? `${workflow?.character_reference_sheet?.reference_images?.length || 0} views`
                    : hasScript && !characterRequired
                        ? 'Skipped (no lead character)'
                        : undefined
            },
            {
                id: 'image',
                ...
                status: hasImages
                    ? 'completed'
                    : characterStepCompleted && workflow?.current_hitl_gate === 'images'
                        ? 'waiting'
                        : characterStepCompleted && canAdvance
                            ? 'running'
                            : 'pending',
                meta: hasImages ? `${imageUrls.length} generated` : undefined
            },
            {
                id: 'video',
                label: 'Video',
                status: hasVideo
                    ? 'completed'
                    : hasImages && workflow?.current_hitl_gate === 'video'
                        ? 'waiting'
                        : hasImages && canAdvance
                            ? 'running'
                            : 'pending'
            }
        ] as const
                        ...
                        <CardContent className="space-y-4">
                            <div className="p-4 bg-muted/30 rounded-lg space-y-4">
                                ...
                                {(() => {
                                    ...
                                    const strategyUsed = workflow.research_output.strategy_used
                                        ...
                                        <div className="pb-4 border-b">
                                            <div className="mb-3">
                                                <span className="text-sm font-semibold">
                                                    {strategyUsed === 'creative'
                                                        ? 'Creative ideation (no sources)'
