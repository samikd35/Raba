...
export default function VPSPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleBackToProject = useCallback(async () => {
    ...
    router.push(`/workspace/solution-critic/${projectId}`);
  }, [router, projectId]);
...
const handleRegenerate = useCallback(async () => {
...
}, [isAuthenticated, token, projectId, router, generateVPS, selectedPersonaId]);
  ...
  const handleContinueToBMC = useCallback(async () => {
    ...
    router.push(`/workspace/bmc-v2/${projectId}`);
  }, [router, projectId]);
