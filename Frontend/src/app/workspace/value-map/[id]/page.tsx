...
export default function ValueMapPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getPersonaValidationMessage = useCallback(() => {
  ...
  const saveSelections = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        router.push(`/workspace/vpc-v2/${projectId}`);
      } else {
  ...
  const handleBackToProject = useCallback(() => {
    router.push(`/workspace/customer-profile-v2/${projectId}`);
  }, [router, projectId]);
                  ...
                  >
                    ...
                    >
                      ...
                      <CardContent className="space-y-3 -mt-4">
                        ...
                        {/* Connections */}
                        {'creates_gain' in item && item.creates_gain.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            <span className="text-xs text-gray-500 dark:text-gray-400">Creates gains:</span>
                            {item.creates_gain.map((gainId, idx) => (
                              <Badge key={idx} variant="outline" className="text-xs bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-600">
                                {gainId}
                              </Badge>
                            ))}
                          </div>
                        ...
                        {'addresses_pain' in item && item.addresses_pain.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            <span className="text-xs text-gray-500 dark:text-gray-400">Addresses pains:</span>
                            {item.addresses_pain.map((painId, idx) => (
                              <Badge key={idx} variant="outline" className="text-xs bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-600">
                                {painId}
                              </Badge>
                            ))}
                          </div>
                        ...
                        {'addresses_jtbd' in item && item.addresses_jtbd.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            <span className="text-xs text-gray-500 dark:text-gray-400">Addresses JTBD:</span>
                            {item.addresses_jtbd.map((jtbdId, idx) => (
                              <Badge key={idx} variant="outline" className="text-xs bg-brand-50 dark:bg-brand-900/30 border-brand-200 dark:border-brand-600">
                                {jtbdId}
                              </Badge>
                            ))}
                          </div>
