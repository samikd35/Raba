...
export default function ProjectPage() {
  ...
  const handleContinueProject = useCallback(() => {
    ...
    const stepRoutes: Record<string, string> = {
      'customer_profile': `/workspace/customer-profile?project_id=${project.id}`,
      'vpc_composition': `/workspace/vpc-composition?project_id=${project.id}`,
      'field_prep': `/workspace/field-prep?project_id=${project.id}`,
      'data_collection': `/workspace/data-collection?project_id=${project.id}`,
      'analysis': `/workspace/analysis?project_id=${project.id}`
    };
    
    const route = stepRoutes[project.current_step] || `/workspace/customer-profile?project_id=${project.id}`;
    router.push(route);
  }, [project, router]);
  ...
  const handleNextPage = useCallback(() => {
    ...
    try {
      ...
      router.push(`/workspace/vps/${projectId}`);
    } catch (err) {
                ...
                <div className="space-y-6">
                  ...
                  {project.field_prep_data.assumptions.length > 3 && (
                    <div className="text-center">
                      ...
                      <Button
                        onClick={() => router.push(`/workspace/assumptions/${projectId}`)}
                        variant="outline"
                ...
                <div className="space-y-6">
                  ...
                  {project.field_prep_data.hypotheses.length > 3 && (
                    <div className="text-center">
                      ...
                      <Button
                        onClick={() => router.push(`/workspace/hypothesis/${projectId}`)}
                        variant="outline"
                        className="dark:border-brand-600 dark:text-brand-300 dark:hover:bg-brand-800"
                      >
                        View All Hypotheses
                ...
                <div className="space-y-6">
                  ...
                  {project.field_prep_data.questionnaires.length > 5 && (
                    <div className="text-center">
                      ...
                      <Button
                        onClick={() => router.push(`/workspace/questionnaires/${projectId}`)}
                        variant="outline"
                        className="dark:border-brand-600 dark:text-brand-300 dark:hover:bg-brand-800"
                      >
                        View All Questions
