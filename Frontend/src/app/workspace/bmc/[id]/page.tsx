...
export default function BMCPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const featureConfig = getFeatureVideoConfig(FEATURE_IDS.BMC);
  ...
  return (
    <div>
      {/* <PageBreadcrumb pageTitle={projectName ? `${projectName} - Business Model Canvas` : "Business Model Canvas"} /> */}
      <FeatureVideoOverlay
        featureId={FEATURE_IDS.BMC}
        youtubeId={featureConfig.youtubeId}
        resourcesHref={featureConfig.resourcesHref}
        title={featureConfig.title}
      />
      <PageBreadcrumb pageTitle={"Construct Your Business Model Canvas"} titleSuffix={<CreditCostBadge cost={20} />}/>
