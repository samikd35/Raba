...
export default function CustomerProfilePage({
  ...
  const loadSelections = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes("404") || err.message.includes("not found")) {
        ...
        router.push(`/workspace/generate-customer-profile/${projectId}`);
      } else {
  ...
  const handleBackToProject = () => {
    router.push(`/workspace/personas/${projectId}`);
  };
  ...
  const handleContinueToVPC = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/workspace/vpc/${projectId}`);
    } catch (err) {
            ...
            {!hasSelections ? (
              <div className="text-center py-12">
                ...
                <Button
                  onClick={() =>
                    router.push(
                      `/workspace/generate-customer-profile/${projectId}`
                    )
                  }
                  className="bg-brand-600 hover:bg-brand-700 dark:bg-brand-500 dark:hover:bg-brand-600"
                >
